<template>
  <div class="h-screen flex flex-col bg-[var(--theme-bg-secondary)]">
    ...
    <header class="short:hidden bg-gradient-to-r from-[var(--theme-primary)] to-[var(--theme-primary-light)] shadow-lg border-b-2 border-[var(--theme-primary-dark)]">
      <div class="px-3 py-4 mobile:py-1.5 mobile:px-2 flex items-center justify-between mobile:gap-2">
        ...
        <div class="flex items-center mobile:space-x-1 space-x-2">
          <span v-show="activeTab === 'events'" class="text-base mobile:text-xs text-white font-semibold drop-shadow-md bg-[var(--theme-primary-dark)] mobile:px-2 mobile:py-0.5 px-3 py-1.5 rounded-full border border-white/30">
            {{ events.length }}
          </span>
    ...
    <!-- Tab bar -->
    <div class="flex bg-[var(--theme-bg-primary)] border-b border-[var(--theme-border)] px-3 short:hidden">
      <button
        v-for="tab in tabs"
        :key="tab.id"
      ...
      >
        ...
        <span
          ...
          class="absolute bottom-0 left-0 right-0 h-0.5 bg-[var(--theme-primary)]"
        ></span>
      </button>
    </div>
...
const activeTab = ref<'events' | 'reports'>('events');
