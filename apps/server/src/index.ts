...
import { getProjectsWithReports, loadProjectsConfig, discoverReports, getReportContent } from './projects';
...
const server = Bun.serve({
  ...
  async fetch(req: Request) {
    ...
    // GET /api/projects - List all projects with their reports
    if (url.pathname === '/api/projects' && req.method === 'GET') {
      const projects = getProjectsWithReports();
      return new Response(JSON.stringify({ projects }), {
        headers: { ...headers, 'Content-Type': 'application/json' }
      });
    }

    // GET /api/projects/:name/reports - Get reports for a specific project
    if (url.pathname.match(/^\/api\/projects\/[^/]+\/reports$/) && req.method === 'GET') {
      const name = decodeURIComponent(url.pathname.split('/')[3]);
      const configs = loadProjectsConfig();
      const project = configs.find(p => p.name === name);
      if (!project) {
      ...
      const reports = discoverReports(project.path);
      return new Response(JSON.stringify({ reports }), {
        headers: { ...headers, 'Content-Type': 'application/json' }
      });
    }

    // GET /api/projects/:name/report-content?path=<relative> - Get report HTML content
    if (url.pathname.match(/^\/api\/projects\/[^/]+\/report-content$/) && req.method === 'GET') {
      const name = decodeURIComponent(url.pathname.split('/')[3]);
      ...
      const html = getReportContent(project.path, relativePath);
      if (html === null) {
        return new Response(JSON.stringify({ error: 'Report not found or path not allowed' }), {
          status: 404,
          headers: { ...headers, 'Content-Type': 'application/json' }
        });
      }
      ...
      if (html.length > 10 * 1024 * 1024) {
        return new Response(JSON.stringify({ error: 'Report too large (>10MB)' }), {
          status: 413,
          headers: { ...headers, 'Content-Type': 'application/json' }
        });
      }
      return new Response(JSON.stringify({ html, name: relativePath.split('/').pop() }), {
        headers: { ...headers, 'Content-Type': 'application/json' }
      });
    }
